...
locals {
  vm_name = "windows-server-2025-${lower(var.build_edition)}"
...
source "proxmox-iso" "windows-server-2025" {
  ...
  tags                 = "Windows;Windows-Server-2025;Windows-Server-2025-${var.build_edition}"
  template_description = "Windows Server 2025 ${var.build_edition}, Built ${timestamp()}"
  ...
  additional_iso_files {
    type         = "sata"
    index        = 2
    iso_file     = "${var.proxmox_iso_storage_pool}:iso/virtio-win-0.1.285-1.iso"

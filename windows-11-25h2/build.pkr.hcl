...
locals {
  vm_name = "windows-11-25h2-${lower(var.build_edition)}"
...
source "proxmox-iso" "windows-11-25h2" {
  # Proxmox connection
  ...
  tags                 = "Windows;Windows-11;Windows-11-${var.build_edition};Windows-11-25H2;Windows-11-25H2-${var.build_edition}"
  template_description = "Windows 11 25H2 ${var.build_edition}, Built ${timestamp()}"
  ...
  efi_config {
    ...
    efi_type          = "4m"
    pre_enrolled_keys = var.vm_secure_boot
  }
  rng0 {
    source    = "/dev/urandom"
    max_bytes = 1024
...
build {
  sources = ["source.proxmox-iso.windows-11-25h2"]

...
locals {
  vm_name = "windows-server-${var.server_version}-${lower(var.build_edition)}"
  vm_id   = var.build_vm_id != "" ? var.build_vm_id : var.edition_build_map[var.build_edition].default_vm_id
}
...
source "proxmox-iso" "windows-server" {
  ...
  tags                 = "Windows;Windows-Server-${var.server_version};Windows-Server-${var.server_version}-${var.build_edition}"
  template_description = "Windows Server ${var.server_version} ${var.build_edition}, Built ${timestamp()}"
    ...
    cd_content       = {
      "Autounattend.xml" = templatefile(
        "${path.root}/Autounattend.xml.tmpl",
        {
          ...
          driver_folder        = replace(var.server_version, "0", "k") # e.g. 2016 -> 2k16
          image_index          = var.edition_build_map[var.build_edition].image_index
          product_key          = var.product_key_list[var.edition_build_map[var.build_edition].product_key]
          computer_name        = "SRV-${var.server_version}-${lookup({"standard"="STD","datacenter"="DATA","essentials"="ESS"}, replace(lower(var.build_edition), "-core", ""), "UNK")}"
          ...
          first_logon_commands = var.build_first_logon_commands
        }
      )
    }
    cd_files = [
